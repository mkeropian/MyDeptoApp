...
export class FormComponent implements OnInit {
  ...
  onAvatarChange(event: Event): void {
    ...
    if (!validTypes.includes(file.type)) {
      this.notificationService.mostrarNotificacion('Solo se permiten archivos de imagen (JPG, PNG, GIF, WEBP)', 'error');
      return;
    }
    ...
    if (file.size > maxSize) {
      this.notificationService.mostrarNotificacion('El archivo no debe superar los 5MB', 'error');
      return;
    }
  ...
  onSubmit() {
    ...
    this.propietariosService.createPropietario(formValue as Propietario).subscribe({
      ...
      error: (error) => {
        ...
        this.notificationService.mostrarNotificacion('Error al crear el propietario', 'error');
      }
    });
  }
  ...
  private uploadAvatar(file: File, propietarioId: number): void {
    ...
    this.propietariosService.uploadAvatarPropietario(file, propietarioId).subscribe({
      ...
      error: (error: any) => {
        ...
        this.notificationService.mostrarNotificacion('Propietario creado pero falló la carga del avatar', 'error');
        this.finalizarCreacion();
      }
    });
  }
  ...
  private finalizarCreacion(): void {
    ...
    this.notificationService.mostrarNotificacion('Propietario creado exitosamente', 'success');
  }
}
